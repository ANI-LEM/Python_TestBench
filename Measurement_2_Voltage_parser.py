...
def example(resource_name, channels, options, voltage1, voltage2, delay):
    ...
    with nidcpower.Session(resource_name=resource_name, channels=channels, options=options) as session:
        ...
        #    print('Voltage 1:')
        #    print_fetched_measurements(session.fetch_multiple(count=1, timeout=timeout))
        #    session.voltage_level = voltage1  # on-the-fly set
        #    print('Voltage 2:')
        #    print_fetched_measurements(session.fetch_multiple(count=1, timeout=timeout))
        #    session.output_enabled = True
...
def _main(argsv):
    ...
    parser.add_argument('-n', '--resource-name', default='PXI1Slot6', help='Resource name of a National Instruments SMU')
    parser.add_argument('-c', '--channels', default='0', help='Channel(s) to use')
    parser.add_argument('-v1', '--voltage1', default=2.0, type=float, help='Voltage level 1 (V)')
...
def test_main():
    cmd_line = ['--option-string', 'Simulate=1, DriverSetup=Model:4112; BoardType:PXIe', ]
    _main(cmd_line)
